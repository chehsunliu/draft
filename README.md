...
The project is designed to support multiple backend languages behind a shared API spec. For now, the implementations are **Go (Gin)**, **Rust (Axum)**, **Java (Spring Boot 4)**, and **Kotlin (Ktor)**; more may be added later.
...
The pytest integration suite picks which backend to build and run via the `ITX_LANG` env var (`rust` by default, or `golang`, `java`, or `kotlin`). The fixtures handle building the binary and starting/stopping the server — you don't need to run anything in `itx-rs/`, `itx-go/`, `itx-java/`, or `itx-kt/` yourself.
