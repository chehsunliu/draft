...
public final class PostRoutes {
    ...
    public static void register(Javalin app, ObjectMapper mapper, AppState state) {
        ...
        app.post("/api/v1/posts", ctx -> {
            ItxContext c = ItxContext.from(ctx);
            CreatePostRequest body = mapper.readValue(ctx.body(), CreatePostRequest.class);
            PostDto dto = state.createPost.execute(
                    new CreatePostUseCase.ExecuteParams(c.userId, body.title(), body.body(), body.tags()));
            ctx.status(201).json(dto);
        });

        app.get("/api/v1/posts/{id}", ctx -> {
            ItxContext c = ItxContext.from(ctx);
        ...
        app.patch("/api/v1/posts/{id}", ctx -> {
            ItxContext c = ItxContext.from(ctx);
            long id = Long.parseLong(ctx.pathParam("id"));
            UpdatePostRequest body = mapper.readValue(ctx.body(), UpdatePostRequest.class);
            PostDto dto = state.updatePost.execute(
                    new UpdatePostUseCase.ExecuteParams(id, c.userId, body.title(), body.body(), body.tags()));
            ctx.json(dto);
        });

        app.delete("/api/v1/posts/{id}", ctx -> {
            ItxContext c = ItxContext.from(ctx);
            long id = Long.parseLong(ctx.pathParam("id"));
