...
public final class Module {
    ...
    public static Javalin build(AppState state, ObjectMapper mapper) {
        ...
        app.exception(BackendException.class, (e, ctx) -> ctx.status(e.status()).result(e.getMessage()));
        app.exception(RepoNotFoundException.class, (e, ctx) -> ctx.status(404).result("not found"));
        app.exception(QueueException.class, (e, ctx) -> ctx.status(500)
                .result(e.getMessage() == null ? "queue error" : e.getMessage()));
        app.exception(Exception.class, (e, ctx) -> ctx.status(500)
                .result(e.getMessage() == null ? "internal error" : e.getMessage()));
        ...
        app.error(404, ctx -> {
            ...
            var available = MethodNotAllowedUtil.INSTANCE.findAvailableHttpHandlerTypes(
