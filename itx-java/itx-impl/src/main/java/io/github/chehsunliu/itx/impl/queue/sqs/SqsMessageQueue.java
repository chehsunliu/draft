...
public final class SqsMessageQueue implements MessageQueue {
    ...
    public void receive(MessageHandler handler, AtomicBoolean cancel) {
        ...
        ExecutorService workers = Executors.newThreadPerTaskExecutor(
                Thread.ofVirtual().name("sqs-worker-", 0).factory());
        ...
        try {
            while (!cancel.get()) {
                ...
                try {
                    resp = client.receiveMessage(ReceiveMessageRequest.builder()
                            .queueUrl(queueUrl)
                ...
                for (Message msg : resp.messages()) {
                    ...
                    jobs.add(workers.submit(() -> {
                        try {
                            ...
                            client.deleteMessage(b -> b.queueUrl(queueUrl).receiptHandle(msg.receiptHandle()));
                        } catch (Throwable t) {
