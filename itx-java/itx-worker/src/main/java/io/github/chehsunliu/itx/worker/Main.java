        ...
        return switch (mode) {
            case "control" -> {
                ...
                ControlDispatcher dispatcher = new ControlDispatcher(
                        ...
                        new ObjectMapper());
                yield new Wired(
                        List.of(factory.createControlStandardQueue(), factory.createControlPremiumQueue()), dispatcher);
            }
            case "compute" ->
                new Wired(
                        List.of(factory.createComputeStandardQueue(), factory.createComputePremiumQueue()),
                        new ComputeDispatcher());
            default -> throw new IllegalStateException("unknown --mode: " + mode + " (expected control|compute)");
        };
    }
}
